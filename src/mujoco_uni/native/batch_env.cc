...
class BatchEnvPool {
 public:
  ...
  ~BatchEnvPool() {
    ...
    // Models exposed through _from_model_ptr are owned by the cached Python
    // wrapper, so skip manual deletion for those and let the wrappers clear
    // after all worker data has been released.
    for (raw::MjModel* m : owned_models_) {
